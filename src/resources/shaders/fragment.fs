...
void main()
{
    ...
    // use triangle normal more "flat" effect, use normal from vertex shader for smooth
    vec3 tri_normal = normalize(cross(dFdx(mvVertexPos), dFdy(mvVertexPos)));

    totalLight += calcDirectionalLight(directionalLight, mvVertexPos, tri_normal);
    totalLight += calcPointLight(pointLight, mvVertexPos, tri_normal); 
